...
- 合成オッズ高度化
- 深層学習の時系列単独モデル
- 回収専用ラベルだけを単独最適化（厳選・EVゲートとセットで扱う）
...
| | 的中寄り | 回収寄り | **本方針（両立）** |
| --- | --- | --- | --- |
| レース | 堅実レースを選ぶ | 歪みのあるレースも買う | 軸が読めるレースだけ。薄いオッズはケン |
| 券種 | 当てやすい券種を優先 | 期待値 > 1 の組合せ | 質で切替＋床/EVゲート |
| オッズ | 評価の主特徴にしがち | 歪みを狙う | **評価には使わず、購入ゲートに使う** |
| 失敗パターン | 全レース・三連単固定 | 的中崩壊 | 低配当の厚買い、EVなしの穴買い |
...
1. 既存印ロジックはそのままに、**軸堅さ / 下位荒れ** の選別スコアをコード化する  
2. 券種マトリクスを設定ファイル化し、神戸新聞杯型（軸堅・下位荒れ）と小回り中荒れで三連複/三連単ゲートを検証する  
3. Pass レースだけで軸流しシミュレーションを回し、見送り込みの的中率を測る  
4. 余力が出たら印用 Ranker と穴3着候補の根拠特徴を追加する  
